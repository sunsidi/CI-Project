...
public function get_latest_related_events($search,$category,$price,$state,$zipcode){
    ...
    if ($state || $price || $zipcode){
        if(!$zipcode){$zipcode = '%';}
        if(!$state){$state = '%';}
    $sql = 'SELECT * FROM events WHERE e_pricetemp<=? and e_zipcode LIKE ? and e_state LIKE ? and e_private = 0 ORDER BY event_id DESC';
    $query = $this->db->query($sql,array($price,$zipcode,$state));
    $events = $query->result_array();
   }
   
   else{
    $query= $this->db->get_where('events', "e_private = 0");
     $events = $query->result_array();
  ...
  public function get_latest_events($search,$price,$zipcode,$state){
    ...
    $sql = 'SELECT * FROM events WHERE e_private = 0 ORDER BY event_id DESC';
    $query = $this->db->query($sql);
    $events = $query->result_array();
    }
    $related_events= array();
